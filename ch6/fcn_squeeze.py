...
class Fire(chainer.Chain):
    ...
    in_size: input channel 
    s1: first output channel
    e1: second output channel
    e3: third output channel
    """

    def __init__(self, in_size, s1, e1, e3):
        super().__init__()
        with self.init_scope():
            # kernel sizeが1の畳み込みが2つと, same paddingを行う畳み込みが1つ
    ...
    def __call__(self, x):
        """
        conv1 -> elu->{conv1, conv2}->conv1_out+conv2_out
        -> concat along axis chanel -> elu
        """
    ...
    def __init__(self, n_class, in_ch):
        ...
        with self.init_scope():
            ...
            self.score_pool4 = L.Convolution2D(
                256, n_class, 1, stride=1, pad=0)
            self.score_pool9 = L.Convolution2D(
                512, n_class, 1, stride=1, pad=0)

            self.add_layer = L.Convolution2D(
                n_class * 3, n_class, 1, stride=1, pad=0)

            # padding means reduce pixels in deconvolution.
            self.upsample_pool4 = L.Deconvolution2D(
                n_class, n_class, ksize=4, stride=2, pad=1)
            self.upsample_pool9 = L.Deconvolution2D(
                n_class, n_class, ksize=8, stride=4, pad=2)
            self.upsample_final = L.Deconvolution2D(
                n_class, n_class, ksize=16, stride=4, pad=(6, 6))

        self.n_class = n_class
        self.active_learn = False
        self.evaluator = Evaluator(gpu=0, n_class)
